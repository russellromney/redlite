...
impl TursoDb {
    ...
    async fn migrate(&self) -> Result<()> {
        ...
        // Create indexes
        conn.execute("CREATE INDEX IF NOT EXISTS idx_keys_db_key ON keys(db, key)", ())
            .await.map_err(|e| KvError::Other(format!("Migration error: {}", e)))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_hashes_key_id ON hashes(key_id)", ())
            .await.map_err(|e| KvError::Other(format!("Migration error: {}", e)))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_lists_key_id_idx ON lists(key_id, idx)", ())
            .await.map_err(|e| KvError::Other(format!("Migration error: {}", e)))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sets_key_id ON sets(key_id)", ())
            .await.map_err(|e| KvError::Other(format!("Migration error: {}", e)))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_zsets_key_id_score ON zsets(key_id, score)", ())
            .await.map_err(|e| KvError::Other(format!("Migration error: {}", e)))?;
    ...
    pub fn set(&self, key: &str, value: &[u8], expire_seconds: Option<u64>) -> Result<()> {
        self.runtime.block_on(self.set_async(key, value, expire_seconds))
    }
    ...
    async fn set_async(&self, key: &str, value: &[u8], expire_seconds: Option<u64>) -> Result<()> {
        ...
        ).await.map_err(|e| KvError::Other(format!("Prepare error: {}", e)))?;

        let mut rows = stmt.query([db.to_string(), key.to_string()])
        ...
        if let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
            ...
            if key_type != KeyType::String as i32 {
                ...
                conn.execute(
                    ...
                    [String::from_utf8_lossy(value).to_string(), key_id.to_string()],
                ).await.map_err(|e| KvError::Other(format!("Update error: {}", e)))?;

                conn.execute(
                    "UPDATE keys SET updated_at = ?1, expire_at = ?2 WHERE id = ?3",
                    [now.to_string(), expire_at.map(|e| e.to_string()).unwrap_or("NULL".to_string()), key_id.to_string()],
                ).await.map_err(|e| KvError::Other(format!("Update error: {}", e)))?;
        ...
        let mut rows = stmt.query(())
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
        let row = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))?
        ...
        conn.execute(
            ...
            [key_id.to_string(), String::from_utf8_lossy(value).to_string()],
        ).await.map_err(|e| KvError::Other(format!("Insert error: {}", e)))?;
    ...
    async fn get_async(&self, key: &str) -> Result<Option<Vec<u8>>> {
        ...
        let mut rows = stmt.query([db.to_string(), key.to_string(), (KeyType::String as i32).to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;

        if let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
            let value: String = row.get(0).map_err(|e| KvError::Other(format!("Get error: {}", e)))?;
    ...
    async fn hset_async(&self, key: &str, pairs: &[(&str, &[u8])]) -> Result<i64> {
        ...
        // Get or create key
        let mut stmt = conn.prepare("SELECT id, type FROM keys WHERE db = ?1 AND key = ?2")
            .await.map_err(|e| KvError::Other(format!("Prepare error: {}", e)))?;
        let mut rows = stmt.query([db.to_string(), key.to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
        ...
        for (field, value) in pairs {
            ...
            let mut stmt = conn.prepare("SELECT 1 FROM hashes WHERE key_id = ?1 AND field = ?2")
                .await.map_err(|e| KvError::Other(format!("Prepare error: {}", e)))?;
            let mut rows = stmt.query([key_id.to_string(), field.to_string()])
                .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;

            let is_new = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))?.is_none();
            drop(rows);
            drop(stmt);
            ...
            if is_new {
                conn.execute(
                    ...
                    [key_id.to_string(), field.to_string(), String::from_utf8_lossy(value).to_string()],
                ).await.map_err(|e| KvError::Other(format!("Insert error: {}", e)))?;
                new_count += 1;
            } else {
                conn.execute(
                    "UPDATE hashes SET value = ?1 WHERE key_id = ?2 AND field = ?3",
                    [String::from_utf8_lossy(value).to_string(), key_id.to_string(), field.to_string()],
                ).await.map_err(|e| KvError::Other(format!("Update error: {}", e)))?;
            }
        }
    ...
    async fn hget_async(&self, key: &str, field: &str) -> Result<Option<Vec<u8>>> {
        ...
        let mut rows = stmt.query([db.to_string(), key.to_string(), field.to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
    ...
    async fn hgetall_async(&self, key: &str) -> Result<Vec<(String, Vec<u8>)>> {
        ...
        while let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
            ...
            let value: String = row.get(1).map_err(|e| KvError::Other(format!("Get error: {}", e)))?;
    ...
    async fn lpush_async(&self, key: &str, values: &[&[u8]]) -> Result<i64> {
        ...
        // Get current min idx
        let mut stmt = conn.prepare("SELECT MIN(idx) FROM lists WHERE key_id = ?1")
            .await.map_err(|e| KvError::Other(format!("Prepare error: {}", e)))?;
        let mut rows = stmt.query([key_id.to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
        let min_idx: f64 = if let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
        ...
        for value in values {
            ...
            conn.execute(
                ...
                [key_id.to_string(), idx.to_string(), String::from_utf8_lossy(value).to_string()],
            ).await.map_err(|e| KvError::Other(format!("Insert error: {}", e)))?;
        }

        // Get new count
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM lists WHERE key_id = ?1")
            .await.map_err(|e| KvError::Other(format!("Prepare error: {}", e)))?;
        let mut rows = stmt.query([key_id.to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
        let count: i64 = if let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
    ...
    async fn lpop_async(&self, key: &str, count: Option<usize>) -> Result<Vec<Vec<u8>>> {
        ...
        let mut rows = stmt.query([db.to_string(), key.to_string(), count.to_string()])
            .await.map_err(|e| KvError::Other(format!("Query error: {}", e)))?;
    ...
    async fn sadd_async(&self, key: &str, members: &[&[u8]]) -> Result<i64> {
        ...
        for member in members {
            ...
            let result = conn.execute(
                ...
                [key_id.to_string(), String::from_utf8_lossy(member).to_string()],
            ).await;
    ...
    async fn smembers_async(&self, key: &str) -> Result<Vec<Vec<u8>>> {
        ...
        let mut result = Vec::new();
        while let Some(row) = rows.next().await.map_err(|e| KvError::Other(format!("Row error: {}", e)))? {
            let member: String = row.get(0).map_err(|e| KvError::Other(format!("Get error: {}", e)))?;
    ...
    async fn zadd_async(&self, key: &str, members: &[(f64, &[u8])]) -> Result<i64> {
        ...
        for (score, member) in members {
            ...
            conn.execute(
                ...
                [key_id.to_string(), String::from_utf8_lossy(member).to_string(), score.to_string()],
            ).await.map_err(|e| KvError::Other(format!("Insert error: {}", e)))?;
            added += 1;
        }
    ...
    async fn incr_async(&self, key: &str) -> Result<i64> {
        ...
        let value = match current {
            Some(bytes) => {
                ...
                s.parse::<i64>().map_err(|_| KvError::Other("value is not an integer".to_string()))?
            }
            None => 0,
        };
